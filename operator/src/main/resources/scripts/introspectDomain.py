...
class TopologyGenerator(Generator):
  ...
  def addServer(self, server, is_server_template=False):
    ...
    self.writeln("  listenAddress: " + self.quote(self.env.toDNS1123Legal(server.getName())))
...
class SitConfigGenerator(Generator):
  ...
  def customizeServer(self, server):
    name=server.getName()
    listen_address=self.env.toDNS1123Legal(name)
  ...
  def customizeServerTemplate(self, template):
    ...
    listen_address=self.env.toDNS1123Legal(server_name_prefix + "${id}")
