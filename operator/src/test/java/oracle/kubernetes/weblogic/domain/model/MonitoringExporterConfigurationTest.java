...
class MonitoringExporterConfigurationTest {
  ...
  void deserializeFromJson() {
    ...
    assertThat(configuration.getConfiguration(), hasJsonPath("$.metricsNameSnakeCase", equalTo(true)));
    assertThat(configuration.getConfiguration(), hasJsonPath("$.queries[0].applicationRuntimes.key", equalTo("name")));
